    ...
    async runEvaluation(topK = 10) {
        ...
        for (const goldQuery of this.goldStandardQueries) {
            ...
            try {
                ...
                const foundRelevantFiles = uniqueActualFiles.filter(file => relevantFiles.some(expected => file.includes(expected) || expected.includes(file)));
                ...
                const missedFiles = relevantFiles.filter(expected => !uniqueActualFiles.some(actual => actual.includes(expected) || expected.includes(actual)));
                const unexpectedFiles = uniqueActualFiles.filter(actual => !relevantFiles.some(expected => actual.includes(expected) || expected.includes(actual)));
                const result = {
                    query: goldQuery.query,
...
export { SearchEvaluationHarness };
