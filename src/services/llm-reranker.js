export class LLMRerankerService {
     ...
     */
    async rerank(request, requestStartTime = Date.now()) {
        if (!this.enabled) {
    ...
    buildRerankingPrompt(request) {
        const candidates = request.candidates.map((result, index) => {
            ...
            const fileKind = result.chunk.fileKind ||
...
JSON Response:`;
     ...
     */
    async callAnthropicAPI(prompt, requestStartTime) {
     ...
     */
    async callOpenAIAPI(prompt, requestStartTime) {
        const controller = new AbortController();
        const remainingTime = this.timeoutMs - (Date.now() - requestStartTime);
        const timeoutMs = Math.max(1000, remainingTime);
        const timeout = setTimeout(() => controller.abort(), timeoutMs);
     ...
     */
    parseRerankingResponse(response, candidates, maxResults) {
        try {
