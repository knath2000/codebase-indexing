...
export class SearchService {
    constructor(config) {
        ...
        this.qdrantClient = new QdrantVectorClient(config.qdrantUrl, config.qdrantApiKey, config.collectionName, this.voyageClient.getEmbeddingDimension(config.embeddingModel));
    ...
    buildSearchQuery(args) {
        ...
        if (args.chunkType !== undefined) {
            searchQuery.chunkType = args.chunkType;
        }
        if (args.filePath !== undefined) {
            searchQuery.filePath = args.filePath;
        }
        if (args.limit !== undefined) {
            searchQuery.limit = args.limit;
        }
        if (args.threshold !== undefined) {
            searchQuery.threshold = args.threshold;
        }
        if (args.enableHybrid !== undefined) {
            searchQuery.enableHybrid = args.enableHybrid;
        }
        if (args.enableReranking !== undefined) {
            searchQuery.enableReranking = args.enableReranking;
        }
        if (args.llmRerankerTimeoutMs !== undefined) {
            searchQuery.llmRerankerTimeoutMs = args.llmRerankerTimeoutMs;
        }
        if (args.maxFilesPerType !== undefined) {
            searchQuery.maxFilesPerType = args.maxFilesPerType;
        }
        if (args.preferFunctions !== undefined) {
            searchQuery.preferFunctions = args.preferFunctions;
        }
        if (args.preferClasses !== undefined) {
            searchQuery.preferClasses = args.preferClasses;
        }
        if (args.preferImplementation !== undefined) {
            searchQuery.preferImplementation = args.preferImplementation;
        }
        return searchQuery;
    }
    /**
    ...
    async search(query) {
        ...
        try {
            ...
            const queryVector = await this.voyageClient.generateEmbedding(query.query, this.config.embeddingModel, 'query' // Use 'query' input type for search queries
            );
     ...
     */
    async getSuggestions(query, type = 'any') {
        const searchQuery = {
            ...query,
            limit: query.limit || 5,
    ...
    async getChunkById(chunkId) {
        try {
            ...
            if (searchResults.length > 0) {
                const chunkPayload = searchResults[0].payload;
                return {
     ...
     */
    async getCodeContext(chunkId, contextLines = 5) {
        try {
            ...
            const nearbyChunks = sortedChunks.filter(c => Math.abs(c.startLine - chunk.startLine) <= contextLines * 10);
            // Combine context
    ...
    async getHealthStatus() {
        ...
        let qdrantStatus = { status: 'unhealthy', lastCheck: timestamp, message: 'Not initialized' };
        let voyageStatus = { status: 'unhealthy', lastCheck: timestamp, message: 'Not initialized' };
        let llmRerankerStatus = { status: 'disabled', lastCheck: timestamp, message: 'LLM Reranker is disabled' };
        let fileWatcherStatus = { status: 'healthy', lastCheck: timestamp, message: 'File watcher not directly managed by SearchService' };
        try {
            const qdrantTest = await this.qdrantClient.testConnection();
     ...
     */
    boostImplementationResults(results) {
     ...
     */
    async searchForCodeReferences(query, maxTokens) {
        ...
        const { references, contextWindow, truncated } = this.contextManager.formatAsCodeReferences(results, maxTokens);
